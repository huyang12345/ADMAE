...
def get_args_parser():
    ...
    parser.add_argument('--mvtec',default=['bottle'],type=str)
    ...
    parser.add_argument('--output_dir', default='./new_train_one/{}',
                        help='path where to save, empty for no saving')
    parser.add_argument('--log_dir', default='./new_train_one/{}',
...
def main(args):
    ...
    # define the model
    model = models_mae_new.__dict__[args.model](norm_pix_loss=args.norm_pix_loss)
    checkpoint = torch.load("./PretrainWeight/mae_visualize_vit_base.pth", map_location='cpu')
...
if __name__ == '__main__':
    ...
    if isinstance(args.mvtec,str):
        temp = []
        temp.append(args.mvtec)
        args.mvtec = [i for i in temp]
    if len(args.mvtec) == 1:
        args.output_dir = args.output_dir.format(args.mvtec[0])
        args.log_dir = args.log_dir.format(args.mvtec[0])
    if args.output_dir:
