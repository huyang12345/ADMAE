...
class MaskedAutoencoderViT(nn.Module):
    ...
    def forward_loss(self, imgs, pred, mask):
        ...
        imgs: [N, 3, H, W]
        pred: [NK, L, p*p*3]
        mask: [NK, L], 0 is keep, 1 is remove,
        target: [N, L, D]
        """
        data_range = 4.5696
        weight = 0.15
        ...
        pred_cons = pred_cons.reshape(-1, target.shape[0], pred.shape[1], pred.shape[2]).sum(0) / (
                    K - 1)
        ms_ssim_batch_wise = 1 - ms_ssim(self.unpatchify(pred_cons), self.unpatchify(target), data_range=data_range,
                                         size_average=True, win_size=11)
        loss = weight * ms_ssim_batch_wise + (1 - weight) * l2_loss_rec

        return loss ,pred_cons

    def forward(self, imgs, mask_ratio=0.75):
        latent, mask, ids_restore = self.forward_encoder(imgs, mask_ratio)
        pred_tem = self.forward_decoder(latent, ids_restore)  # [N, L, p*p*3]
        loss, pred = self.forward_loss(imgs, pred_tem, mask)
