...
class MaskedAutoencoderViT(nn.Module):
    ...
    def random_masking(self,x, mask_ratio):
        ...
        x_nmasked = torch.zeros([K, N, int(L / K), D], device=x.device)
        mask = torch.zeros([K, N, L], device=x.device)
        if mask_ratio < 0.5:
            K = int(1 / mask_ratio)
            x_nmasked = torch.zeros([K, N, int(L-L / K), D], device=x.device)
            mask = torch.zeros([K, N, L], device=x.device)
        for i in range(K):
            step = int(L / K)
            ids = ids_shuffle[:, i * step:(i + 1) * step]
            if mask_ratio < 0.5:
                ids = torch.cat([ids_shuffle[:, 0:i * step],ids_shuffle[:,(i + 1) * step:]],dim=1)
            x_nmasked[i] = torch.gather(x, dim=1, index=ids.unsqueeze(-1).repeat(1, 1, D))  # select patch
        ...
        x_nmasked = x_nmasked.reshape(-1, int(L / K), D)
        if mask_ratio < 0.5:
            x_nmasked = x_nmasked.reshape(-1, int(L-L / K), D)
        ...
        return x_nmasked, mask, ids_restore
    ...
    def forward_decoder(self, x, ids_restore):
        ...
        step = int(L / K)
        for i in range(K):#存疑，是否此mask图片的同一部份,但根据mask_torkens维度看都是同一个维度
            x_tem = torch.cat((mask_tokens[:,0:i*step,:],x_[i][:, 1:, :], mask_tokens[:,0:(K-i)*step,:]), dim=1)  # no cls token
    ...
    def forward_loss(self, imgs, pred, mask,cls_d):
        ...
        lpips_loss = loss_fn.forward(self.unpatchify(pred_cons), self.unpatchify(target))
        ...
        #loss = 0.25* l2_loss_rec + 0.5 * lpips_loss.mean() +0.25*  ms_ssim_batch_wise
        loss = 0.5 * lpips_loss.mean()
        return loss ,pred_cons,#[l2_loss_rec,lpips_loss.mean() ,ms_ssim_batch_wise]
    ...
    def forward(self, imgs, mask_ratio=0.75):
        ...
        loss,pred1 = self.forward_loss(imgs, pred_tem, mask,cls_d)
        if torch.isnan(loss).any():
            print(1)
        return loss,pred1,mask
