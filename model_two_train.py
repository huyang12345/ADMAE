...
import models_mae
import model_two
from engine_model_two import train_one_epoch, evaluate

# from engine_pretrain_original import train_one_epoch,evaluate
# from engine_pretrain1 import train_one_epoch,evaluate
...
def get_args_parser():
    parser = argparse.ArgumentParser('a_shap_warm10_24_mask_ratio0.75_capsule_test', add_help=False)
    parser.add_argument('--batch_size', default=8, type=int,
                        help='Batch size per GPU (effective batch size is batch_size * accum_iter * # gpus')
    parser.add_argument('--epochs', default=300, type=int)
    parser.add_argument('--accum_iter', default=1, type=int,
                        help='Accumulate gradient iterations (for increasing the effective batch size under memory constraints)')
    parser.add_argument('--mvtec', default=[ 'metal_nut',], type=list)
    ...
    parser.add_argument('--output_dir', default='./h',
                        help='path where to save, empty for no saving')
    parser.add_argument('--log_dir', default='./h',
...
def main(args):
    ...
    # define the model
    model = model_two.__dict__[args.model](norm_pix_loss=args.norm_pix_loss)
    model_svdd = model_two.__dict__['vit_base_patch16']()
    # checkpoint = torch.load("./shap_warm10_24_mask_ratio0/checkpoint-100.pth", map_location='cpu')
    # checkpoint_model = checkpoint['model']
    # model.load_state_dict(checkpoint_model, strict=False)
    # print('load model ok')
    model.to(device)
    model_svdd.to(device)
    model_without_ddp = model
    model_without_ddp_1 = model_svdd
    ...
    param_groups_1 = optim_factory.add_weight_decay(model_without_ddp_1, args.weight_decay)

    optimizer = torch.optim.AdamW(param_groups, lr=args.lr, betas=(0.9, 0.95))  # todo

    optimizer_1 = torch.optim.AdamW(param_groups_1, lr=args.lr, betas=(0.9, 0.95))
    print(optimizer)
    print(optimizer_1)
    loss_scaler = NativeScaler()

    misc.load_model(args=args, model_without_ddp=model_without_ddp, optimizer=optimizer, loss_scaler=loss_scaler)
    misc.load_model(args=args, model_without_ddp=model_without_ddp_1, optimizer=optimizer_1, loss_scaler=loss_scaler)

    print(f"Start training for {args.epochs} epochs")
    start_time = time.time()
    model_svdd.c = init_c(args, data_loader_train, model_svdd)
    model_svdd.sigma = init_sigma(args, model_svdd.c, data_loader_train, model_svdd)
    print('init ok')
    for epoch in range(args.start_epoch, args.epochs):
        if args.output_dir and (epoch % 20 == 0 or epoch + 1 == args.epochs):
            auc_file = open(args.output_dir + "/auc.txt", "a")
            evaluate(args, model, model_svdd, device, epoch, auc_file)
            auc_file.close()
        train_stats = train_one_epoch(
            model, model_svdd, data_loader_train,
            optimizer,
            optimizer_1,
        ...
        if args.output_dir and (epoch % 50 == 0 or epoch + 1 == args.epochs):
            ...
            # misc.save_model(
            #     args=args, model=model_svdd, model_without_ddp=model_without_ddp_1, optimizer=optimizer_1,
            #     loss_scaler=loss_scaler, epoch=epoch)
