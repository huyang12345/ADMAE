...
class MaskedAutoencoderViT(nn.Module):
    ...
    def random_masking(self, x, mask_ratio):
        ...
        K = int(1 / (mask_ratio)) # 针对mask_ratio小于0.5的情况
        x_unmasked = torch.zeros([K, N, L - int(L / K), D], device=x.device)
        mask = torch.zeros([K, N, L], device=x.device)
        for i in range(K):
            step = int(L / K)
            ids = torch.cat((ids_shuffle[:,:i * step],ids_shuffle[:,(i + 1) * step:]),dim=1)
            x_unmasked[i] = torch.gather(x, dim=1, index=ids.unsqueeze(-1).repeat(1, 1, D))  # select patch
        ...
        x_unmasked = x_unmasked.reshape(-1, L - int(L / K), D)
    ...
    def forward_decoder(self, x, ids_restore):
        ...
        step = int(L / K)
        for i in range(K):
            x_tem = torch.cat((x_[i][:, 1:i * step+1, :], mask_tokens[:, :, :], x_[i][:, i * step+1:, :]),
                              dim=1)  # no cls token
    ...
    def forward_loss(self, imgs, pred, mask, cls_d):
        ...
        l1_loss_rec = (pred - target.repeat(K, 1, 1)) ** 2
        l1_loss_rec = l1_loss_rec.mean(-1)
        # l1_loss_rec = l1_loss_rec.mean()
        l1_loss_rec = (l1_loss_rec * mask).sum() / mask.sum()
        ...
        loss = 0.5 * (weight * ms_ssim_batch_wise + (1 - weight) * l1_loss_rec)

        # loss = 0.8*l1_loss_rec + 0.2*ms_CGS

        # loss = 0.8*loss + 0.2*loss_svd

        # loss = 0.5* l1_loss_rec + 0.5*loss
        return loss, pred
